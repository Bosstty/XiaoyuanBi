      ...
      Object.keys(data).forEach((key) => {
        if (data[key] !== undefined && data[key] !== null) {
          ...
          if (Array.isArray(data[key])) {
            data[key].forEach((val) => searchParams.append(key, val))
          } else {
...
export const orderManagementApi = {
  ...
  // 取消订单
  cancelOrder: (id) => apiClient.delete(`/admin/orders/${id}`),
  // 获取订单统计
...
export const taskManagementApi = {
  ...
  // 更新任务状态
  updateTaskStatus: (id, status) => apiClient.patch(`/admin/tasks/${id}/status`, { status }),
  // 删除任务
...
export const delivererManagementApi = {
  ...
  // 删除/封禁配送员
  deleteDeliverer: (id) => apiClient.delete(`/admin/deliverers/${id}`),
  // 获取配送员统计
...
export const forumManagementApi = {
  ...
  // 审核帖子
  moderatePost: (id, action) => apiClient.patch(`/admin/forum/posts/${id}/moderate`, action),
  // 删除帖子
  deletePost: (id) => apiClient.delete(`/admin/forum/posts/${id}`),
  // 获取评论列表
  getComments: (params) => apiClient.get('/admin/forum/comments', params),
  // 删除评论
  deleteComment: (id) => apiClient.delete(`/admin/forum/comments/${id}`),
  // 获取举报列表
  getReports: (params) => apiClient.get('/admin/forum/reports', params),
  // 处理举报
  handleReport: (id, action) => apiClient.patch(`/admin/forum/reports/${id}`, action),
}
