...
async function generateTestData() {
  try {
    ...
    const ticketData = [
      {
        ...
        priority: 'high',
        content: '今天下午点了一份外卖，配送员到达后态度非常差，还说了脏话，希望平台能重视这个问题。'
      },
      {
        type: 'refund',
        title: '订单退款申请',
      ...
      },
      {
        ...
        priority: 'low',
        content: '晚上学习到很晚有时候需要点外卖，但很多店夜间不配送，建议平台能解决这个问题。'
      },
      {
        type: 'complaint',
        title: '快递丢失',
    ...
    for (let i = 0; i < ticketData.length; i++) {
      ...
      const randomDeliverer = deliverers.length > 0 ? deliverers[Math.floor(Math.random() * deliverers.length)] : null;
      const randomOrder = orders.length > 0 ? orders[Math.floor(Math.random() * orders.length)] : null;
      ...
      console.log(`创建工单成功: ${data.title}, 工单号: ${ticket.ticket_no}, 状态: ${status}`);
    }
    ...
    for (let i = 0; i < extraTickets.length; i++) {
      ...
      const ticket = await ServiceTicket.create({
        ...
        user_id: randomUser.id,
        service_id: serviceId,
        solution: data.status === 'resolved' || data.status === 'closed' ? '已处理完成' : null,
        resolved_at: data.status === 'resolved' || data.status === 'closed' ? new Date() : null
      });
